...
pub const WindowsWindow = ManagedWindow;

pub const WindowState = struct {
    uses_win32_window: bool = true,
    next_handle: usize = 1,
    ...
    pub fn open(self: *WindowState, allocator: std.mem.Allocator, options: common.WindowOptions) !usize {
        const handle = self.next_handle;
        self.next_handle += 1;

        var managed = try ManagedWindow.init(allocator, handle, options);
    ...
    pub fn close(self: *WindowState, allocator: std.mem.Allocator, handle: usize) !void {
        ...
        if (self.active_handle != null and self.active_handle.? == handle) {
            self.active_handle = if (self.windows.items.len == 0) null else self.windows.items[self.windows.items.len - 1].handle;
    ...
    fn syncFocus(self: *WindowState, active_handle: usize) void {
        for (self.windows.items) |*managed| managed.active = managed.handle == active_handle;
    }
};

pub const WindowsWindowState = WindowState;
